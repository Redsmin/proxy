  ...
  _.extend(RedisClient.prototype, require('events').EventEmitter.prototype, {
    ...
    updatePortAndHostname: function (uri) {
      ...
      if (uri[0] !== '/') { // not a socket path
        ...
        this.isTLS = (parsedUri.protocol + '//' === this.TLS_PREFIX);
      } else {
    ...
    _connect: function () {
      ...
      if (this.port && this.hostname) {
        // port
        this.socket = this.isTLS ? tls.connect(this.port, this.hostname, this.onConnected) : net.createConnection(this.port, this.hostname, this.onConnected);
      } else {
        // socket
